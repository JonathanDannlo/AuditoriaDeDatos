...
# Calculo de la matricula en el DataFrame Inner Academico Financiero
...
df_piam20241Cii = pd.merge(PIAM20241CI, facturacion20241[['Documento', 'Id  factura', 'Estado Actual', 'Valor Factura','Valor Ajuste','Valor Pagado','Valor Anulado','Saldo','Nombre de Destino']], left_on='Id  factura', right_on='Id  factura', how='left')
...
with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
    ...
    filtro_estadofinanciero.to_excel(writer, sheet_name='Generalidades', startrow=1, startcol=1, index=False)
    filtro_estado.to_excel(writer, sheet_name='Generalidades', startrow=1, startcol=5, index=False)
    filtro_estadofinancieroSQ.to_excel(writer, sheet_name='Generalidades', startrow=1, startcol=10, index=False)
    ...
    #registrosDuplicadosPiamfi.to_excel(writer, sheet_name='DuplicadosXBoleta', index=False)
    #registrosDuplicadosIDSnPiamfi.to_excel(writer, sheet_name='DuplicadosXIdSnies', index=False)
