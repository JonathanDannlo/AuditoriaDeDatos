...
# Verifica qué registros no cumplen con la condición
#registrosInvalidosTipoID = df_piam20241_Cidl_SinDuplicados[~df_piam20241_Cidl_SinDuplicados['TIPOIDENTIFICACION'].isin(valoresvalidosTipoId)]
#if 'TIPOIDENTIFICACION' in registrosInvalidosTipoID.columns:
    #registrosInvalidosTipoID['TIPOIDENTIFICACION'] = registrosInvalidosTipoID['TIPOIDENTIFICACION'].str.strip().str.upper()
    #registrosInvalidosTipoID.loc[~registrosInvalidosTipoID['TIPOIDENTIFICACION'].isin(valoresvalidosTipoId), 'TIPOIDENTIFICACION'] = pd.NA
...
# Obtiene el workbook y el worksheet
  ...
  if not df_piam20241_Cidl_SinDuplicados.empty:
    ...
    if not df_no_validos.empty:
        ...
        print('No se encontraron registros no válidos por tipo de identificacion')
  else:
